...
<template>
    <div>
        <Table :ref="refs" :height="height" :columns="columnsList" :data="thisTableData" border disabled-hover></Table>
    </div>
</template>
    ...
    return h('Poptip', {
        ...
        on: {
            'on-ok': () => {
                ...
                vm.deleteRow(vm.deleteIndex(index), vm.successDel(vm, index), vm.failDel(vm, index),currentRow);
            }
        }
    }, [
        h('Button', {
